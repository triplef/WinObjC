...
#define TAP_SLACK_AREA (((GetCACompositor()->screenWidth() / GetCACompositor()->deviceWidth()) * GetCACompositor()->screenXDpi()) / 3.0f) //  1/3 inch
...
static void RecordTouch(float x, float y, double timeStamp)
{
    ...
    while ( curPos != touchRecordTail ) {
        if ( timeStamp - touchRecords[curPos].timeStamp <= 0.300 && touchDist(x, y, touchRecords[curPos].x, touchRecords[curPos].y) < TAP_SLACK_AREA ) {
            //  Update the record
...
@implementation UIApplication {
    ...
    -(void) notify:(unsigned)activity {
        ...
        if ( _applicationState == UIApplicationStateBackground ) return;
            ...
            if ( curView->_backButtonDelegate != nil ) {
                ...
                for ( int j = windowCount - 1; j >= 0; j -- ) {
                    ...
                    if ( pointView == curView || [pointView isDescendantOfView:curView] ) {
                        ...
                        break;
                    } else if ( pointView != nil && pointView != curWindow ) break;
                }
            }
        }
    ...
    +(void) _doBackAction {
        if ( ignoringInteractionEvents ) return;
        ...
        for (UIView* curView in allActionButtons) {
            NSInvocation *inv = [NSInvocation invocationWithMethodSignature:
                [[curView->_backButtonDelegate class] instanceMethodSignatureForSelector:curView->_backButtonSelector]];
    ...
    -(void) _receiveAlarm:(id)localNotification {
        ...
        if (idx == NSNotFound) return; // probably deleted.

        UIAlertView* alert = [[UIAlertView alloc] initWithTitle:@"Alert" message:[localNotification alertBody] delegate:nil cancelButtonTitle:[localNotification alertAction] otherButtonTitles:nil];
    ...
    -(void) setStatusBarOrientation:(UIInterfaceOrientation)orientation animated:(BOOL)animated {
        ...
        switch ( _curOrientation ) {
            case UIInterfaceOrientationLandscapeRight:
                ...
                [statusBarRotationLayer setCenter:CGPointMake(appFrame.origin.x + appFrame.size.width / 2.0f, appFrame.origin.y + appFrame.size.height / 2.0f)];
                [popupRotationLayer setCenter:CGPointMake(appFrame.origin.x + appFrame.size.width / 2.0f, appFrame.origin.y + appFrame.size.height / 2.0f)];
                rect.origin.x = 0.0f;
                rect.origin.y = 0.0f;
    ...
    -(void) setStatusBarMode:(unsigned)mode orientation:(UIInterfaceOrientation)orientation duration:(unsigned)duration fenceID:(unsigned)fenceID {
    }
    ...
    static void printViews(id curView, int level)
    {
        ...
        for ( int i = 0; i < level * 2; i ++ ) sprintf(&szOut[strlen(szOut)], " ");
    ...
    -(void) newMouseEvent {
        ...
        for ( int curEvent = 0; curEvent < numEvents; curEvent ++ ) {
            ...
            */
            ...
            if ( allTouches == nil ) allTouches = [[NSMutableSet set] retain];
            ...
            switch ( evt->mouseEvent ) {
                case mouseDown: {
                    ...
                    for ( int i = count - 1; i >= 0; i -- ) {
                        ...
                        if ( mouseView != nil && mouseView != curWindow ) break;
                    }

                    if ( mouseView != nil ) EbrDebugLog("%s touched\n", object_getClassName(mouseView));
            ...
            switch ( evt->mouseEvent ) {
                case mouseUp:
                    ...
                    for ( int i = 0; i < 10; i ++ ) {
                        if ( touches[i] != nil ) resetAllTrackingGestures = FALSE;                      
                    }
    ...
    -(BOOL) openURL:(NSURL*)url {
        ...
        if ( [scheme isEqualToString:@"fbauth"] ) return FALSE;
    ...
    -(void) endIgnoringInteractionEvents {
        if ( ignoringInteractionEvents > 0 ) ignoringInteractionEvents --;
    }
    ...
    static void layoutBlankView(UIView* inputView, UIView* accessoryView, float totalHeight)
    {
        if ( totalHeight > 0 ) {
            ...
            if ( _curKeyboardAccessory != accessoryView ) {
                ...
                if ( _curKeyboardAccessory != nil ) [_blankView addSubview:_curKeyboardAccessory];
            }
            ...
            if ( _curKeyboardInputView != inputView ) {
                ...
                if ( _curKeyboardInputView != nil ) [_blankView addSubview:_curKeyboardInputView];
            }

            if ( _curKeyboardInputView != nil ) {
                CGRect keyboardRect = { 0, accessorySize.size.height,
                    statusRect.size.width, totalHeight - accessorySize.size.height };
    ...
    static void animateKeyboardResize(id self, float newHeight, bool forceKeyboardAppearance)
    {
        ...
        id values[7] = {
            [NSValue valueWithCGRect:mappedEnd],
            [NSValue valueWithCGRect:mappedStart],
            [NSNumber numberWithDouble:0.25],
            [NSNumber numberWithInt:0],
            [NSValue valueWithCGRect:keyboardBounds],
            [NSValue valueWithCGPoint:centerBegin],
        ...
        if ( newHeight > 0 ) {
            ...
            if ( !blankViewUp ) {
                ...
                if ( oldHeight != newHeight ) {
                    [[NSNotificationCenter defaultCenter] postNotificationName:@"UIKeyboardWillChangeFrameNotification" object:nil userInfo:dict];
                    [[NSNotificationCenter defaultCenter] postNotificationName:@"UIKeyboardDidChangeFrameNotification" object:nil userInfo:dict];
                }
            }
        } else {
            //  Keyboard is being hidden
    ...
    /* annotate with type */ -(id) _keyboardChanged {
    ...
    // The textfield the keyboard is for has changed state and we need to figure out what to do:
    /* annotate with type */ -(id) _evaluateKeyboard {
        evaluateKeyboard(self);
        return self;
    }
    ...
    -(void) _didRegisterForRemoteNotification:(NSData*)tokenData {
        ...
        id alertView = [[UIAlertView alloc] initWithTitle:@"GCM Registration Succeeded" message:errStr delegate:nil defaultButton:@"Ok" cancelButton:nil otherButtons:nil];
        [alertView show];
        [alertView release];
        [str release];
        */
    ...
    /*
        ...
        id alertView = [[UIAlertView alloc] initWithTitle:@"GCM Registration Failed" message:errStr delegate:nil defaultButton:@"Ok" cancelButton:nil otherButtons:nil];
        [alertView show];
        [alertView release];
        */
    }
    ...
    -(void) _didReceiveRemoteNotification:(NSData*)data {
        ...
        id alertView = [[UIAlertView alloc] initWithTitle:@"GCM Message Received" message:errStr delegate:nil defaultButton:@"Ok" cancelButton:nil otherButtons:nil];
        [alertView show];
        [alertView release];
        [str release];
        */
...
void UIQueueTouchInput(float x, float y, int fingerID, int eventType, float surfaceWidth, float surfaceHeight, int64_t eventTime, bool bLandscape)
{
    int touchID = 0;
    ...
    if ( x < 0.0f ) x = 0.0f;
    if ( x > GetCACompositor()->screenWidth() ) x = GetCACompositor()->screenWidth();
    if ( y < 0.0f ) y = 0.0f;
    if ( y > GetCACompositor()->screenHeight() ) y = GetCACompositor()->screenHeight();
